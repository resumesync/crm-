...
import { Gift, Send, Calendar, Cake, Loader2 } from 'lucide-react';
...
export default function Birthdays() {
  const [leads, setLeads] = useState<BirthdayLead[]>([]);
  ...
  useEffect(() => {
    const fetchBirthdays = async () => {
      try {
        // Fetch all leads that have birthdays
        const data = await apiFetch('/api/birthdays');
        setLeads(data || []);
      } catch (error) {
        console.error('Failed to load birthday leads:', error);
        // Fallback: try fetching from leads endpoint
        try {
          const data = await apiFetch('/api/leads?per_page=10000');
          const withBirthday = (data.leads || []).filter((l: any) => l.birthday);
          setLeads(withBirthday);
        } catch {
          setLeads([]);
        }
      } finally {
  ...
  const thisMonthBirthdays = leadsWithBirthdays.filter(l => {
    if (!l.birthday) return false;
    const bday = new Date(l.birthday);
    return isSameMonth(new Date(today.getFullYear(), bday.getMonth(), bday.getDate()), today);
  });

  const sendBirthdayWish = (lead: BirthdayLead) => {
    const name = (lead.full_name || 'there').split(' ')[0];
    const clinic = lead.clinic_name || 'our clinic';
    const message = `Dear ${name}, wishing you a very Happy Birthday! 🎂🎉\n\nMay this special day bring you joy, health, and happiness. Thank you for being a valued member of our ${clinic} family.\n\nWith warm wishes,\nAbhivrudhi Agency`;
          ...
          <div className="mb-8">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {todayBirthdays.map((lead, index) => (
                <Card key={lead.id} className="animate-fade-in border-accent/30 bg-accent/5 p-4" style={{ animationDelay: `${index * 60}ms` }}>
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-accent/20">
                      <span className="text-xl">🎂</span>
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-foreground">{lead.full_name || 'Anonymous'}</p>
                      <p className="text-sm text-muted-foreground">{lead.clinic_name || '-'}</p>
                    </div>
                  </div>
                  <Button variant="default" size="sm" className="mt-4 w-full bg-accent hover:bg-accent/90" onClick={() => sendBirthdayWish(lead)}>
          ...
          <div className="mb-8">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {upcomingBirthdays.map((lead, index) => (
                <Card key={lead.id} className="animate-fade-in p-4 transition-all hover:shadow-medium" style={{ animationDelay: `${index * 60}ms` }}>
                  <div className="flex items-center gap-3">
                  ...
                  <p className="mt-3 text-sm text-muted-foreground">
                    📅 {lead.birthday && format(new Date(today.getFullYear(), new Date(lead.birthday).getMonth(), new Date(lead.birthday).getDate()), 'EEEE, MMMM d')}
                  </p>
                  <Button variant="outline" size="sm" className="mt-3 w-full" onClick={() => sendBirthdayWish(lead)}>
                    <Send className="h-4 w-4" />
                    Schedule Wish
                  </Button>
                </Card>
          ...
          {thisMonthBirthdays.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {thisMonthBirthdays.map((lead, index) => (
                <Card key={lead.id} className="animate-fade-in p-4" style={{ animationDelay: `${index * 40}ms` }}>
                  <div className="flex items-center gap-2">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10">
                      <span className="text-sm font-medium text-primary">
                        {lead.birthday && new Date(lead.birthday).getDate()}
                      </span>
                    </div>
                    <div className="flex-1 overflow-hidden">
                      <p className="truncate font-medium text-foreground">{lead.full_name || 'Anonymous'}</p>
