...
export function LeadDetailPanel({ lead, onClose }: LeadDetailPanelProps) {
  ...
  const fetchNotes = async () => {
    try {
      const notes = await apiFetch(`/api/notes/lead/${lead.id}`);
      setEditedLead(prev => ({
        ...prev,
  ...
  const handleSave = async () => {
    try {
      await apiFetch(`/api/leads/by-id/${lead.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
          full_name: editedLead.name,
          phone_number: editedLead.phone,
          email: editedLead.email,
          clinic_name: editedLead.clinicName,
          city: editedLead.city,
          state: editedLead.state,
          pincode: editedLead.pincode,
          address: editedLead.address,
          status: editedLead.status,
          assigned_owner_id: editedLead.assignedTo ? parseInt(editedLead.assignedTo) : null,
          age: editedLead.age,
          gender: editedLead.gender,
          birthday: editedLead.birthday ? format(editedLead.birthday, 'yyyy-MM-dd') : null,
        })
      });
      toast({
        title: "Lead Updated",
  ...
  const handleAddNote = async () => {
    ...
    try {
      const response = await apiFetch(`/api/notes/lead/${lead.id}`, {
        method: 'POST',
      ...
      const newNoteObject = {
        ...
        createdBy: String(response.created_by),
      };
